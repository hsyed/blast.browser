...
object JxBrowserManager {
    ...
    private val JX_BROWSER_VERSION = "6.6"
    ...
    fun initializeJxBrowser(type: BrowserType): Browser {
        ...
        if (ChromiumExtractor.create().shouldExtract(BrowserPreferences.getChromiumDir())) {
            ...
            val jxBrowserLicenseUrl = pluginEmbeddedOrSandboxJarFileUrl(JX_BROWSER_LICENSE_ARTIFACT)

            println(jxBrowserArtifactUrl)
            println(jxBrowserLicenseUrl)

            val cl = URLClassLoader(arrayOf(
                    URL(platformArtefact),
                    URL(jxBrowserLicenseUrl),
